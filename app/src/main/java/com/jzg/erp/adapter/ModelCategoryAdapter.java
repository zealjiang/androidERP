...
public class ModelCategoryAdapter extends BaseAdapter {
    ...
    public boolean isEnabled(int position) {
        if (groupNames.contains(((Model) getItem(position)).getName()) &&
                Constant.IS_TITLE.equals(((Model) getItem(position)).getManufacturerName())) {
            return false;
        } else {
            return true;
        }
    ...
    public int getItemViewType(int position) {
        return groupNames.contains(((Model) getItem(position)).getName()) &&
                Constant.IS_TITLE.equals(((Model) getItem(position)).getManufacturerName()) ? TYPE_SEPARATOR : TYPE_ITEM;
    }
