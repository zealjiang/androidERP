...
public class UpdateManager {
    ...
    public void checkAppUpdate(final Context context, final boolean isShowMsg) {
        ...
        map.put("phoneType", "androidphone");
        Map<String, Object> maps = new HashMap<>();
        maps.putAll(map);
        map.put("sign", MD5Utils.getMD5Sign(maps));

        JzgApp.getApiServer();
        JzgApp.getApiServer().isUpdate(map)
            ...
            public void onNext(UpdateApp updateApp) {
                ...
                if (updateApp.getStatus() == 1) {
                    mUpdate = updateApp;
                    UpdateApp.SourceEntity sourceEntity;
                    if(updateApp.getSource()==null){
                        sourceEntity = updateApp.getData();
                    }else{
                        sourceEntity = updateApp.getSource();
                    }

                    if (curVersionCode < sourceEntity.getVersionCode()) {
                        apkUrl = sourceEntity.getDownloadUrl();
                        updateMsg = sourceEntity.getUpdateLog();
                        String upDateForce = sourceEntity.getUpdateForce();
    ...
    private Runnable mdownApkRunnable = new Runnable() {
        ...
        public void run() {
            try {
                ...
                UpdateApp.SourceEntity sourceEntity;
                if(mUpdate.getSource()==null){
                    sourceEntity = mUpdate.getData();
                }else{
                    sourceEntity = mUpdate.getSource();
                }

                String apkName = "JZGJC_" + sourceEntity.getVersionName() + ".apk";
                String tmpApk = "JZGJC_" + sourceEntity.getVersionName() + ".tmp";
                //判断是否挂载了SD卡
                String storageState = Environment.getExternalStorageState();
                if (storageState.equals(Environment.MEDIA_MOUNTED)) {
                    savePath = Environment.getExternalStorageDirectory().getAbsolutePath() + "/JZGErShouChe/Update/";
